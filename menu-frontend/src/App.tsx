...
export default function App() {
  ...
  if (loading || !catalog || products.length === 0 || !currentProduct) {
    return (
  ...
  return (
    <main className="relative h-screen w-screen overflow-hidden bg-white">
      {/* CAPA DE IMÁGENES: Renderizamos todas para que la transición sea fluida */}
        ...
        >
          {product.imageUrl && (
            <img
              ...
              alt={product.name}
              className="h-full object-cover mx-auto" // object-cover asegura la proporción
            />
          )}
        </div>
      ))}
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-between px-[5vw] py-[5vh]">
        <div
          key={`text-${currentProduct.id}`}
          className="max-w-[70vw] animate-fade-in-up"
        >
            ...
            {products.map((_: any, index: number) => (
              <div
                ...
                className={`h-2 rounded-full bg-white transition-all duration-500 ${
                  index === currentIndex ? "w-12 opacity-100" : "w-8 opacity-35"
                }`}
              />
            ))}
          </div>
